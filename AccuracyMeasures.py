"""
Accuracy Measures for clustering output of RTKM, KMOR, and NEO-k-means as in "Robust Trimmed k-means".
"""
...
def label_to_weight_mat(label: np.ndarray, num_points: int, num_clusters: int):
    """
    Create weight matrix from labels.

    Args:
        label (np.ndarray): Array of clustering labels.
        num_points (int):
        num_clusters:
    """
...
def rocdist(true_outliers: np.ndarray, pred_outliers: np.ndarray, n: int) -> float:
    ...
    Args:
        ...
        pred_outliers (np.ndarray):  Array containing 0 for points that are predicted to be outliers and 1 for points that are not.
...
def align_labels(true_clusters: np.ndarray, pred_clusters: np.ndarray) -> np.ndarray:
    ...
    Args:
        ...
        pred_clusters (np.ndarray) Predicted point assignment weight matrix of size kxN (should not have outliers attached).

    Returns:
        current_assignment (np.ndarray): Array storing the indices that align the rows of the predicted clusters with
        those of the true clusters in order to maximize the f1 score.
    """
...
def final_clusters(true_clusters: np.ndarray, pred_clusters: np.ndarray,
                   membership_option: Literal['single', 'multi'], pred_outliers: Optional[np.ndarray] = None) -> np.ndarray:
    """
    ...
    Returns:
        final_pred_clusters (np.ndarray): Final predicted clusters for output ordered so that F1 score is maximized for each cluster.
...
def sensitivity_to_alpha(data: np.ndarray, k: int, kmor_gamma: float, neo_sigma: float, path_to_neo: 'str',
                         alpha_vals: np.ndarray, iterations: int, true_clusters: np.ndarray,
                         membership_option: Literal['single', 'multi'], rtkm_sigma: int = 1.0):
    """
    ...
    Args:
        ...
        kmor_gamma (float): KMOR parameter specifying the extent of outlier detection.
        neo_sigma (float): NEO-k-means parameter controlling how many point assignments are made (1 + sigma)n
        path_to_neo (str): String path to the location of the neo-k-means code on the computer.
        alpha_vals (np.ndarray): Values of the predicted percentage of outliers to test.
        iterations (int): Number of iterations each method is run using a particular value of alpha.
        true_clusters (np.ndarray): Matrix of true point assignment weights with size kxN if there are no outliers
        and k+1xN if there are outliers. Outlier assignments should be in the final row of true_clusters.
        membership_option (Literal['single', 'multi']: String specifying 'single' or 'multi' depending on whether each
        point only belongs to one cluster or can belong to multiple clusters.
    ...
    Returns:
        f1_avg_rtkm (np.ndarray): Array containing average f1 score for RTKM over iterations for each value of alpha.
        f1_avg_kmor (np.ndarray): Array containing average f1 score for KMOR over iterations for each value of alpha.
        f1_avg_neo (np.ndarray): Array containing average f1 score for NEO-k-means over iterations for each value of alpha.
        me_avg_rtkm (np.ndarray): Array containing average me score for RTKM over iterations for each value of alpha.
        me_avg_kmor (np.ndarray): Array containing average me score for KMOR over iterations for each value of alpha.
        me_avg_neo (np.ndarray): Array containing average me score for NEO-k-means over iterations for each value of alpha.
        min_rtkm (np.ndarray): Array with first row containing minimum f1 scores over each of the iterations for each value of alpha
            and second row containing minimum me scores over each of the iterations for each value of alpha.
        max_rtkm (np.ndarray): Array with first row containing minimum f1 scores over each of the iterations for each value of alpha
            and second row containing minimum me scores over each of the iterations for each value of alpha.
        min_kmor (np.ndarray): Array with first row containing minimum f1 scores over each of the iterations for each value of alpha
            and second row containing minimum me scores over each of the iterations for each value of alpha.
        max_kmor (np.ndarray): Array with first row containing minimum f1 scores over each of the iterations for each value of alpha
            and second row containing minimum me scores over each of the iterations for each value of alpha.
        min_neo (np.ndarray): Array with first row containing minimum f1 scores over each of the iterations for each value of alpha
            and second row containing minimum me scores over each of the iterations for each value of alpha.
        max_neo (np.ndarray): Array with first row containing minimum f1 scores over each of the iterations for each value of alpha
            and second row containing minimum me scores over each of the iterations for each value of alpha.
    """
    assert membership_option in ['single', 'multi'], "Not a valid membership option."
    ...
    for i, alpha_val in enumerate(alpha_vals):
        ...
        for j in range(iterations):
            ...
            rtkm_wbc = RTKM(data)
            rtkm_wbc.perform_clustering(k, percent_outliers=alpha_val, max_iter=1000, init_centers=init_centers,
                                        num_members=rtkm_sigma)
            tp, fp, tn, fn, f1_rtkm, me_rtkm = accuracy_measures(true_clusters=true_clusters,
            ...
            tp, fp, tn, fn, f1_kmor, me_kmor = accuracy_measures(true_clusters=true_clusters,
            ...
            tp, fp, tn, fn, f1_neo, me_neo = accuracy_measures(true_clusters=true_clusters,
