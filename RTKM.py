...
def SimplexProx(z, a):
    ...
    ---------
    x : np.ndarray
        Variable to be projected. Assume ``x`` is a matrix, each row will be projected onto a simplex.
    a: float
        Simplex to be projected onto 
    Returns
    -------
    np.ndarray
        Projected variable.
    """
    # sort x in the descending order
    u = z.copy()
    
    #if u is an array, add an empty dimmension 
    if u.ndim == 1:
        u = u[np.newaxis,:]
